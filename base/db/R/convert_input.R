...
convert_input <-
  function(input.id,
    ...
    PEcAn.logger::logger.debug(paste(
      "convert_inputs", fcn, input.id, host$name, outfolder, formatname,
    ...
    PEcAn.logger::logger.info(paste(
      "start CHECK convert_inputs", fcn, input.id, host$name, outfolder,
    ...
    if (forecast) {
      ...
      if (!is.integer(ensemble)) {
        ensemble <- as.integer(1)
      }
      ...
      for (i in seq_len(ensemble)) {
        ...
        existing.dbfile[[i]] <- dbfile.input.check(
          siteid = site.id,
          mimetype = mimetype,
        ...
        if (nrow(existing.dbfile[[i]]) > 0) {
          ...
          existing.machine <- db.query(paste0(
            "SELECT * from machines where id  = '",
            existing.dbfile[[i]]$machine_id, "'"
          ), con)
          ...
          if (existing.machine$id == machine$id) {
            if (overwrite) { # If the files are on the current machine, check to see if we should overwrite them.
              ...
              files.to.delete <- c(files.to.delete, as.list(PEcAn.remote::remote.execute.R(
                paste0(
                  "list.files('",
                ...
                host,
                user = NA, verbose = TRUE, R = Rbinary, scratchdir = outfolder
              )))
      ...
      if (nrow(existing.dbfile) > 0) {
        ...
        if (overwrite) {
          ...
          files.to.delete <- PEcAn.remote::remote.execute.R(
            paste0(
              "list.files('",
    ...
    if (!is.null(browndog) && host$name == "localhost") {
      ...
      if (!is.null(browndog$username) && !is.null(browndog$password)) {
        curloptions$userpwd <- paste(
          browndog$username, browndog$password,
          sep = ":"
        )
    ...
    if (write) {
      ...
      for (i in 1:length(result)) { # Master for loop
        ...
        if (exists("existing.input") && nrow(existing.input[[i]]) > 0 &&
          ...
          db.query(
            paste0(
              "UPDATE inputs SET start_date='", start_date, "', end_date='",
        ...
        if (overwrite) {
          ...
          if (exists("existing.input") && nrow(existing.input[[i]]) > 0) {
            db.query(paste0(
              "UPDATE inputs SET name='", basename(dirname(result[[i]]$file[1])),
