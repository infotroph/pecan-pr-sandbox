...
test_that("transformstats works", {
  expect_equal(
    signif(transformstats(test.stats)$stat, 5),
...
test_that("arrhenius scaling works", {
  expect_equivalent(
    signif(arrhenius.scaling(1, 2, 3), 5),
    c(1.0403)
  )
  expect_equivalent(
    signif(arrhenius.scaling(1, 3, 2), 5),
    c(0.96129)
  )
})
...
test_that("as.sequence works", {
  ...
  expect_equal(
    as.sequence(c("a", NA), na.rm = FALSE),
    c(1, NA)
  )
...
test_that("mstmipvar works with args specified", {
  ...
  expect_s3_class(
    mstmipvar("NPP", lat = lat, lon = lon, time = time, nsoil = nsoil),
    "ncvar4"
  )
})
