...
write.config.FATES <- function(defaults, trait.values, settings, run.id) {
  ...
  for (i in seq_len(npft)) {
    ...
    for (v in seq_along(pft)) {
      ...
      if (var == "Vcmax") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_vcmax25top", start = ipft, count = 1,
          vals = pft[v]
        ) ## (umol CO2 m-2 s-1)
      }
      # Ball-Berry slope
      if (var == "stomatal_slope.BB") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_BB_slope", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "cuticular_cond") {
        if (photo_flag == 0) {
          ...
          ncdf4::ncvar_put(
            nc = fates.param.nc, varid = "fates_bbopt_c4", start = 1, count = 1,
            vals = pft[v]
          )
        } else if (photo_flag == 1) {
          PEcAn.logger::logger.debug("** Setting C3 cuticular conductance value")
          ncdf4::ncvar_put(
            nc = fates.param.nc, varid = "fates_bbopt_c3", start = 1, count = 1,
            vals = pft[v]
          )
        } else {
      ...
      if (var == "Ha_Modified_Arrhenius_Vcmax") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_vcmaxha", start = ipft, count = 1,
          vals = pft[v] * 1000
      ...
      if (var == "Hd_Modified_Arrhenius_Vcmax") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_vcmaxhd", start = ipft, count = 1,
          vals = pft[v] * 1000
        ) ## convert from kj/mol to J/mol (FATES units)
      }
      ...
      if (var == "Ha_Modified_Arrhenius_Jmax") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_jmaxha", start = ipft, count = 1,
          vals = pft[v] * 1000
        ) ## convert from kj/mol to J/mol (FATES units)
      }
      ...
      if (var == "Hd_Modified_Arrhenius_Jmax") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_jmaxhd", start = ipft, count = 1,
          vals = pft[v] * 1000
        ) ## convert from kj/mol to J/mol (FATES units)
      }
      ...
      if (var == "deltaS_Vcmax") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_vcmaxse", start = ipft, count = 1,
          vals = pft[v]
        ) ## convert from kj/mol to J/mol (FATES units)
      }
      ...
      if (var == "deltaS_Jmax") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_jmaxse", start = ipft, count = 1,
          vals = pft[v]
        ) ## convert from kj/mol to J/mol (FATES units)
      }
      ### ----- Leaf physiological parameters
      ...
      if (var == "r_fract") { ## Fraction of carbon balance remaining after maintenance costs have been met that is dedicated to seed production.	[0-1]
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_seed_alloc", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "agf_bs") { ## The fraction of sapwood and structural biomass that is above ground [0-1]
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_allom_agb_frac", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "seed_rain_kgC") { ## External seed rain from outside site (non-mass conserving) ;
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_seed_rain", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ## missing from params.nc
      ...
      if (var == "DBH_at_HTMAX") { ## note in FATES parameter list about switching to HTMAX
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_allom_dbh_maxheight", start = ipft, count = 1,
          vals = pft[v]
        ) ## [cm]
      }
      if (var == "growth_resp_factor") { ## r_growth = grperc * (gpp+r_maint)  fates_grperc:long_name = "Growth respiration factor" ;
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_grperc", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "SLA") { ## default 0.012
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_slatop", start = ipft, count = 1,
      ...
      if (var == "leaf_turnover_rate") { ## fates_leaf_long:long_name = "Leaf longevity (ie turnover timescale)" ;
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_leaf_long", start = ipft, count = 1,
          vals = 1 / pft[v]
      ...
      if (var == "root_turnover_rate") { ## fates_root_long:long_name = "root longevity (alternatively, turnover time)" ;
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_root_long", start = ipft, count = 1,
          vals = 1 / pft[v]
        ) ## root_long = 1/root_turnover_rate, 1/years -> years
      }
      if (var == "c2n_leaf") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_leafcn", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "fineroot2leaf") { # "Allocation parameter: new fine root C per new leaf C" units = "gC/gC"
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_froot_leaf", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "sapwood_ratio") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_allom_latosa_int", start = ipft, count = 1,
          vals = PEcAn.utils::ud_convert(pft[v], "m2 m-2", "m2 cm-2")
        )
      }
      if (var == "leaf_width") { # Characteristic leaf dimension use for aerodynamic resistance
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_dleaf", start = ipft, count = 1,
      ...
      if (var == "hgt_min") { # the minimum height (ie starting height) of a newly recruited plant" ;
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_hgt_min", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "leaf_reflect_nir") { # Leaf reflectance: near-IR	[0-1]
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_rholnir", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "leaf_reflect_vis") { # Leaf reflectance: visible	[0-1]
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_rholvis", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "wood_reflect_nir") { # Stem reflectance: near-IR	[0-1]
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_rhosnir", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "wood_reflect_vis") { # Stem reflectance: visible	[0-1]
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_rhosvis", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "leaf_trans_nir") { # Leaf transmittance: near-IR
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_taulnir", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "leaf_trans_vis") { # Leaf transmittance: visible	pft
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_taulvis", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "wood_trans_nir") { # Stem transmittance: near-IR
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_tausnir", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "wood_trans_vis") { # Stem transmittance: visible
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_tausvis", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "orient_factor") { # Leaf/stem orientation index	[-0/4 <xl< 0.6], fates_xl:valid_range = -1., 1. ;
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_xl", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "wood_density") { # Wood Specific Gravity (ie density of wood relative to density of water),
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_wood_density", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "roota_par") { # CLM rooting distribution parameter [1/m]
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "roota_par", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "rootb_par") { # CLM rooting distribution parameter [1/m]
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "rootb_par", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "clone_alloc") { # A carbon allocation that is added on to seed_alloc for trees larger than dbh_max.	[0-1]
        ...
        ncvar_put(
          nc = fates.param.nc, varid = "fates_clone_alloc", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "storage_target_ratio") { # The target fraction of storage carbon over leaf carbon	[0-1]
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_cushion", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "crown_depth_fraction") { # Crown depth fraction of a cohort relative to its total height [0-1]
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_crown_depth_frac", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "hydraulic_stress_mortality") { # The mortality rate imposed on plants meeting hydraulic stress failure condition [1/yr]
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_stress_mort", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "PPA_comp_exclusion") { # Competetive exclusion parameter for weighting demotions from the upper canopy classification in PPA
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_comp_excln", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "leaf_stor_priority") { # Leaf turnover vs labile carbon use prioritisation. This is the fraction of maintenance demand that
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_leaf_stor_priority", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "understory_treefall_mortality") { # The fraction of trees in understory that die from impacts of large treefalls.
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_understorey_death", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "displar") { # fates_displar:long_name = "Ratio of displacement height to canopy top height"
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_displar", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "z0mr") { # Ratio of momentum roughness length to canopy top height
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_z0mr", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "inital_stem_density") { # Stem density of different PFTs during a bare ground initialization.	[/m2]
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_initd", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "bark_scaler") { # Fraction of tree diameter that is bark. Used in fire.	[0-1]
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_bark_scaler", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "crown_kill") { ## SPITFIRE: Mortality from fire scorching susceptibility parameter
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_crown_kill", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "crop") { # Binary crop flag: 0. = not crop, 1. = crop
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "irrigated") { # Binary Irrigated PFT flag
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "irrigated", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      if (var == "cold_deciduous") { # Binary flag for seasonal-deciduous leaf habit (0-not,1-it is)
        ...
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_season_decid", start = ipft, count = 1,
          vals = pft[v]
        )
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_evergreen", start = ipft, count = 1,
          vals = 0
        )
      }
      if (var == "stress_deciduous") { # Binary flag for stress-deciduous leaf habit (0-not,1-it is)
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_stress_decid", start = ipft, count = 1,
          vals = pft[v]
        )
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_evergreen", start = ipft, count = 1,
      ...
      if (var == "woody") { # Binary woody lifeform flag (0-is not woody, 1-it is woody)
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_woody", start = ipft, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "veg_respiration_Q10") { ## Q10 for maintenance respiration. CLM param. q10_mr(allpfts)
        ncdf4::ncvar_put(
          nc = clm.param.nc, varid = "q10_mr", start = 1, count = 1,
          vals = pft[v]
        )
      }
      if (var == "CelluloseS") { ## Cellulose fraction for CWD
        ncvar_put(
          nc = fates.param.nc, varid = "fates_cwd_fcel", start = 1, count = 1,
          vals = pft[v]
        )
      }
      if (var == "s_lignin") { ## Lignin fraction for CWD
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_cwd_flig", start = 1, count = 1,
          vals = pft[v]
        )
      }
      if (var == "c2n_som1") { ## C:N for SOM pool 1. CLM param
        ncdf4::ncvar_put(
          nc = clm.param.nc, varid = "cn_s1_bgc", start = 1, count = 1,
          vals = pft[v]
        )
      }
      if (var == "c2n_som2") { ## C:N for SOM pool 2. CLM param
        ncdf4::ncvar_put(
          nc = clm.param.nc, varid = "cn_s2_bgc", start = 1, count = 1,
          vals = pft[v]
        )
      }
      if (var == "c2n_som3") { ## C:N for SOM pool 3. CLM param
        ncdf4::ncvar_put(
          nc = clm.param.nc, varid = "cn_s3_bgc", start = 1, count = 1,
          vals = pft[v]
        )
      }
      if (var == "cnscalefactor") { ## Scale factor on CN decomposition for assigning methane flux . CLM param
        ncdf4::ncvar_put(
          nc = clm.param.nc, varid = "cnscalefactor", start = 1, count = 1,
          vals = pft[v]
        )
      }
      ...
      if (var == "CWD_frac1") { ## Fraction of coarse woody debris (CWD) that is moved into each of the four woody fuel classes
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_CWD_frac", start = 1, count = 1,
          vals = pft[v]
        )
      }
      if (var == "CWD_frac2") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_CWD_frac", start = 2, count = 1,
          vals = pft[v]
        )
      }
      if (var == "CWD_frac3") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_CWD_frac", start = 3, count = 1,
          vals = pft[v]
        )
      }
      if (var == "CWD_frac4") {
        ncdf4::ncvar_put(
          nc = fates.param.nc, varid = "fates_CWD_frac", start = 4, count = 1,
          vals = pft[v]
        )
      }
