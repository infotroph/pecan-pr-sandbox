...
write.config.MAAT <- function(defaults = NULL, trait.values, settings, run.id) {
  ...
  system2(
    file.path(settings$model$binary, "run_scripts/setup_MAAT_project.bs"),
    c(
      maat_mod_obj, rundir, file.path(settings$model$binary, "run_scripts"),
  ...
  traits.list <- list()
  maat_param_prefix_list <- list(
    param = c(
      "Ha.", "Hd.", "atref.", "reftemp.", "Topt.", "deltaS.", "a_deltaS_t.", "b_deltaS_t.", "q10.", "a_q10_t.",
