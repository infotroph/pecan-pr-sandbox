...
met2model.DALEC <- function(in.path, in.prefix, outfolder, start_date, end_date,
  ...
  if (!is.null(spin_nyear)) {
    ...
    start_date <- PEcAn.data.atmosphere::spin.met(
      in.path, in.prefix, start_date, end_date,
  ...
  for (year in start_year:end_year) {
    ...
    if (useCO2) {
      CO2 <- CO2 * 1e+06
    } ## convert from mole fraction (kg/kg) to ppm
    ...
    if (length(HydResist) == 1) {
      PEcAn.logger::logger.warn(
        "total plant-soil hydraulic resistance (MPa.m2.s/mmol-1) not specified, setting to default: ",
        HydResist
      )
    ...
    if (length(LeafWaterPot) == 1) {
      PEcAn.logger::logger.warn(
        "maximum soil-leaf water potential difference (MPa) not specified, setting to default: ",
        LeafWaterPot
      )
