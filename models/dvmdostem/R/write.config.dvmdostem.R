...
setup.outputs.dvmdostem <- function(dvmdostem_calibration,
  ...
  if (grepl(tolower(dvmdostem_calibration), "yes", fixed = TRUE)) {
    PEcAn.logger::logger.warn(
      "Calibration run requested! Ignoring requested ",
...
requested_vars_string2list <- function(req_v_str, outspec_path) {
  ...
  req_v_list <- unlist(lapply(unlist(strsplit(req_v_str, ",")), function(x) {
    x[!x == ""]
  }))
...
write.config.dvmdostem <- function(defaults = NULL, trait.values, settings, run.id) {
  ...
  ) # this seems like a problem with below since we copy this first
        ...
        for (i in names(jd)) {
          if (grepl("pft", i)) {
            ...
            if (identical(jd[[i]]$name, pft_common_name)) {
              ...
              if (curr_trait == "cfall_leaf") {
                calparbgc_jsondata[[i]]$`cfall(0)` <- traits[[curr_trait]]
              }
              if (curr_trait == "cfall_stem") {
                calparbgc_jsondata[[i]]$`cfall(1)` <- traits[[curr_trait]]
              }
              if (curr_trait == "cfall_root") {
                calparbgc_jsondata[[i]]$`cfall(2)` <- traits[[curr_trait]]
              }
              if (curr_trait == "nfall_leaf") {
                calparbgc_jsondata[[i]]$`nfall(0)` <- traits[[curr_trait]]
              }
              if (curr_trait == "nfall_stem") {
                calparbgc_jsondata[[i]]$`nfall(1)` <- traits[[curr_trait]]
              }
              if (curr_trait == "nfall_root") {
                calparbgc_jsondata[[i]]$`nfall(2)` <- traits[[curr_trait]]
              }
              if (curr_trait == "krb_leaf") {
                calparbgc_jsondata[[i]]$`krb(0)` <- traits[[curr_trait]]
              }
              if (curr_trait == "krb_stem") {
                calparbgc_jsondata[[i]]$`krb(1)` <- traits[[curr_trait]]
              }
              if (curr_trait == "krb_root") {
                calparbgc_jsondata[[i]]$`krb(2)` <- traits[[curr_trait]]
              }
              if (curr_trait == "kra") {
                calparbgc_jsondata[[i]]$kra <- traits[[curr_trait]]
              }
              if (curr_trait == "frg") {
                calparbgc_jsondata[[i]]$frg <- traits[[curr_trait]]
              }
              if (curr_trait == "nmax") {
                calparbgc_jsondata[[i]]$nmax <- traits[[curr_trait]]
              }
              if (curr_trait == "cmax") {
                calparbgc_jsondata[[i]]$cmax <- traits[[curr_trait]]
              }
              if (curr_trait == "micbnup") {
                calparbgc_jsondata[[i]]$micbnup <- traits[[curr_trait]]
              }
              if (curr_trait == "kdcrawc") {
                calparbgc_jsondata[[i]]$kdcrawc <- traits[[curr_trait]]
              }
              if (curr_trait == "kdcsoma") {
                calparbgc_jsondata[[i]]$kdcsoma <- traits[[curr_trait]]
              }
              if (curr_trait == "kdcsompr") {
                calparbgc_jsondata[[i]]$kdcsompr <- traits[[curr_trait]]
              }
              if (curr_trait == "kdcsomcr") {
                calparbgc_jsondata[[i]]$kdcsomcr <- traits[[curr_trait]]
              }
