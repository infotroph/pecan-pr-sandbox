...
run_BASGRA <- function(run_met, run_params, site_harvest, site_fertilize, start_date, end_date, outdir,
  ...
  mini_met2model_BASGRA <- function(file_path,
    ...
    if (nmw > NMAXDAYS) {
      ...
      PEcAn.logger::logger.info(
        "BASGRA currently runs only", NMAXDAYS,
  ...
  for (y in years) {
    ...
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("stubble_carbon_content",
      units = "kg C m-2", dim = dims, missval = -999,
      longname = "Stubble Carbon Content"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("stem_carbon_content",
      units = "kg C m-2", dim = dims, missval = -999,
      longname = "Stem Carbon Content"
    )
    nc_var[[length(nc_var) + 1]] <- PEcAn.utils::to_ncvar("root_carbon_content", dims)
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("reserve_carbon_content",
      units = "kg C m-2", dim = dims, missval = -999,
      longname = "Reserve Carbon Content"
    )
    nc_var[[length(nc_var) + 1]] <- PEcAn.utils::to_ncvar("leaf_carbon_content", dims)
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("dead_leaf_carbon_content",
      units = "kg C m-2", dim = dims, missval = -999,
    ...
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("nonelongating_generative_tiller",
      units = "m-2", dim = dims, missval = -999,
      longname = "Non-elongating generative tiller density"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("elongating_generative_tiller",
      units = "m-2", dim = dims, missval = -999,
      longname = "Elongating generative tiller density"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("nonelongating_vegetative_tiller",
      units = "m-2", dim = dims, missval = -999,
      longname = "Non-elongating vegetative tiller density"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("phenological_stage",
      units = "-", dim = dims, missval = -999,
      longname = "Phenological stage"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("tiller_density",
      units = "m-2", dim = dims, missval = -999,
    ...
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("SoilMoist",
      units = "kg m-2", dim = dims, missval = -999,
