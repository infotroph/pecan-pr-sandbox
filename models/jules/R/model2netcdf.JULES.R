    ...
    for (i in seq_along(vars)) {
      ...
      if (length(diff_dims) > 0) { # ie more than just x, y, time
        ...
        if (diff_dims %in% c("pft", "type")) {
          ...
          PEcAn.logger::logger.error("Can't properly convert", vars[i])
        }
        ...
        if (nonstd_var) {
          nc_var <- ncdf4::ncvar_def(var,
            units = var_dump$units, dim = list(time = nc$dim$time),
