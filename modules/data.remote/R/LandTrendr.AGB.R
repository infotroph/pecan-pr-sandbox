...
download.LandTrendr.AGB <- function(outdir, target_dataset = "biomass", product_dates = NULL,
  ...
  if (length(files_to_download_final) < 1) {
    ...
    if (run_parallel) {
      ...
      foreach::foreach(i = 1:length(files_to_download_final)) %dopar%
        try(PEcAn.utils::download_file(download_urls_final[i], file.path(
          outdir,
      ...
      Map(function(u, d) PEcAn.utils::download_file(u, d), download_urls_final, file.path(
        outdir,
  ...
  downloaded_years <- unlist(regmatches(
    downloaded_files,
  ...
  out_formats <- c(
    rep("LandTrendr_AGB_median", times = med_rows),
    rep("LandTrendr_AGB_stdev", times = sdev_rows)
  )

  results <- data.frame(
    file = character(total_rows),
...
extract.LandTrendr.AGB <- function(site_info, dataset = "median", buffer = NULL, fun = "mean",
  ...
  if (!is.null(product_dates)) {
    agb_files <- list.files(file.path(data_dir),
      pattern = paste0("*", dataset, ".tif$"),
      full.names = TRUE
    )
    availible_years <- unlist(regmatches(
      agb_files,
  ...
  if (is.null(buffer)) {
    processed_years <- unlist(regmatches(
      names(data.frame(agb_pixel)),
