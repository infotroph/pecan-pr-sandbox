...
download.NLCD <- function(outdir, year = 2011, con = NULL) {
  ...
  if (!is.null(con)) {
    ...
    if (nrow(chk) > 0) {
      machines <- PEcAn.DB::db.query(paste(
        "SELECT * from machines where id in (",
        paste(chk$machine_id, sep = ","), ")"
      ), con)
  ...
  if (!is.null(con)) {
    prefix <- table(sapply(strsplit(dir(data_dir), ".", fixed = TRUE), function(x) {
      x[1]
    }))
    prefix <- names(which.max(prefix))
    site.id <- 1000000676
    return(PEcAn.DB::dbfile.insert(data_dir,
      in.prefix = prefix, type = "Input", input.id, con,
      reuse = TRUE
    ))
  }
  return(data_dir)
  ...
  if (!is.null(con)) {
    ...
    if (nrow(chk) > 0) {
      ...
      if (PEcAn.remote::fqdn() %in% machines$hostname) {
        ...
        print(paste0(
          "File not found on localhost, please check database input.id ",
