...
model.train <- function(dat.subset, v, n.beta, resids = resids, threshold = NULL, ...) {
  ...
  if (v == "surface_downwelling_longwave_flux_in_air") {
    mod.doy <- stats::lm(
      sqrt(surface_downwelling_longwave_flux_in_air) ~
  ...
  if (v == "air_pressure") {
    mod.doy <- stats::lm(
      air_pressure ~ as.ordered(hour) * (air_pressure.day +
  ...
  if (v == "specific_humidity") {
    mod.doy <- stats::lm(
      log(specific_humidity) ~ as.ordered(hour) *
  ...
  if (v == "wind_speed") {
    mod.doy <- stats::lm(
      sqrt(wind_speed) ~ as.ordered(hour) * wind_speed.day *
  ...
  if (is.na(summary(mod.doy)$adj.r.squared)) {
    stop(paste0(
      "Can not estimate covariance matrix for day of year: ",
  ...
  if (resids == TRUE) {
    if (v == "air_temperature") {
      dat.subset[
        !is.na(dat.subset$lag.air_temperature) & !is.na(dat.subset$next.air_temperature_max),
        "resid"
      ] <- stats::resid(mod.doy)
    ...
    if (v == "surface_downwelling_longwave_flux_in_air") {
      dat.subset[
        !is.na(dat.subset$lag.surface_downwelling_longwave_flux_in_air) &
    ...
    if (v == "air_pressure") {
      dat.subset[
        !is.na(dat.subset$lag.air_pressure) & !is.na(dat.subset$next.air_pressure),
        "resid"
      ] <- stats::resid(mod.doy)
    ...
    if (v == "wind_speed") {
      dat.subset[
        !is.na(dat.subset$lag.wind_speed) & !is.na(dat.subset$next.wind_speed),
        "resid"
      ] <- stats::resid(mod.doy)
