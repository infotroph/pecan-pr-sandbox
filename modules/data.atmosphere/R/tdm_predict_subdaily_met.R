...
predict_subdaily_met <- function(outfolder, in.path, in.prefix, path.train, direction.filter = "forward", lm.models.base,
  ...
  vars.hour <- c(
    "air_temperature", "precipitation_flux", "surface_downwelling_shortwave_flux_in_air",
    "surface_downwelling_longwave_flux_in_air", "air_pressure", "specific_humidity",
    "wind_speed"
  )
  vars.lag <- c(
    "lag.air_temperature", "lag.precipitation_flux", "lag.surface_downwelling_shortwave_flux_in_air",
  ...
  for (y in 1:length(yrs.tdm)) {
    ...
    met.out <- align.met(
      train.path = path.train, source.path = in.path,
    ...
    dat.ens <- data.frame(
      year = met.out$dat.source$time$Year,
    ...
    dat.nxt <- data.frame(
      year = met.nxt$dat.train$time$Year,
    ...
    if (direction.filter == "backward") {
      ...
      dat.nxt2 <- data.frame(
        year = met.nxt$dat.train$time$Year[1],
      ...
      dat.nxt2 <- data.frame(
        year = met.nxt$dat.train$time$Year[row.last],
    ...
    ens.sims <- lm_ensemble_sims(
      dat.mod = dat.ens, n.ens = n.ens,
    ...
    time <- ncdf4::ncdim_def(
      name = "time", units = paste0(
        "days since ",
        yrs.tdm[y], "-01-01T00:00:00Z"
      ), vals = as.array(days_elapsed), create_dimvar = TRUE,
      unlim = TRUE
    )
    ...
    for (j in seq_along(nc.info$CF.name)) {
      var.list[[j]] <- ncdf4::ncvar_def(
        name = as.character(nc.info$CF.name[j]),
