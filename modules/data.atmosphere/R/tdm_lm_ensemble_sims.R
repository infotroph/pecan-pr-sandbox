...
lm_ensemble_sims <- function(dat.mod, n.ens, path.model, direction.filter, lags.list = NULL,
  ...
  # downscaling
  dat.info <- c(
    "sim.day", "year", "doy", "hour", "air_temperature_max.day",
  ...
  for (v in vars.list) {
    ...
    for (i in 1:length(days.sim)) {
      ...
      if (v == "surface_downwelling_shortwave_flux_in_air") {
        ...
        hrs.day <- unique(dat.train$time[
          dat.train$time$DOY == day.now &
        ...
        if (i == 1) { # First time through, so pull from our inital lags
          ...
          sim.lag <- utils::stack(data.frame(array(
            dat.sim[["air_temperature"]][dat.mod$sim.day == (days.sim[i - 1]) &
        ...
        if (i == 1) {
          ...
          sim.lag <- utils::stack(data.frame(array(
            dat.sim[[v]][dat.mod$sim.day == days.sim[i - 1] &
              dat.mod$hour == lag.time, ],
      ...
      model.file <- file.path(path.model, v, paste0(
        "model_", v, "_", day.now,
        ".Rdata"
      ))
      ...
      while (n.new > 0 & sane.attempt <= sanity.tries) {
        ...
        # } else {
        dat.pred[, cols.redo] <- subdaily_pred(
          newdata = dat.temp, model.predict = mod.save,
          Rbeta = Rbeta, resid.err = FALSE, model.resid = NULL, Rbeta.resid = NULL,
          n.ens = n.new
        )
        ...
        if (v %in% c("air_temperature", "air_temperature_maximum", "air_temperature_minimum")) {
          ...
          cols.redo <- which(apply(dat.pred, 2, function(x) {
            min(x) < 184 | max(x) > 331 |
        ...
        if (v == "specific_humidity") { # LOG!!
          ...
          cols.redo <- which(apply(dat.pred, 2, function(x) {
            min(exp(x)) < 1e-6 | max(exp(x)) > 3.2e-2 |
        ...
        if (v == "surface_downwelling_shortwave_flux_in_air") {
          ...
          cols.redo <- which(apply(dat.pred, 2, function(x) {
            max(x) > 1500 | min(x) < filter.mean - sanity.sd * filter.sd |
