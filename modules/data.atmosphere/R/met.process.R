...
met.process <- function(site, input_met, start_date, end_date, model,
  ...
  if (!is.null(browndog)) {
    ...
    if (is.data.frame(result)) {
      dbentry <- PEcAn.DB::dbfile.input.insert(
        in.path = dirname(result$file),
  ...
  if (is.null(input_met$id)) {
    ...
    assign(
      ...
      list(
        ...
        dbfile.id = PEcAn.DB::dbfile.check("Input", input_met$id,
          hostname = machine.host, con =
            con
        )$id
      )
    )
  ...
  if (stage$download.raw) {
    ...
    if (nrow(db.file) > 0) {
      ...
      mimetypeid <- PEcAn.DB::get.id(
        table = "mimetypes", colnames = "type_string",
        values = "application/x-netcdf", con = con
      )

      formatid <- PEcAn.DB::get.id(
        table = "formats", colnames = c("mimetype_id", "name"),
  ...
  if (stage$standardize) {
    ...
    for (i in seq_along(cf.id[[1]])) {
      if (register$scale == "regional") {
        #### Site extraction
        standardize_result[[i]] <- .extract.nc.module(
          cf.id = list(
            input.id = cf.id$container_id[i],
        ...
        standardize_result[[i]] <- .metgapfill.module(
          cf.id = list(input.id = cf.id$input.id[i], dbfile.id = cf.id$dbfile.id[i]),
          register = register,
          dir = dir,
          met = met,
          str_ns = str_ns,
  ...
  if (stage$met2model) {
    ...
    for (i in seq_along(ready.id[[1]])) {
      met2model.result[[i]] <- .met2model.module(
        ready.id = list(input.id = ready.id$input.id[i], dbfile.id = ready.id$dbfile.id[i]),
        model = model,
        con = con,
        host = host,
        dir = dir,
...
db.site.lat.lon <- function(site.id, con) {
  site <- PEcAn.DB::db.query(paste(
    "SELECT id, ST_X(ST_CENTROID(geometry)) AS lon, ST_Y(ST_CENTROID(geometry)) AS lat FROM sites WHERE id =",
    site.id
  ), con)
  if (nrow(site) == 0) {
...
browndog.met <- function(browndog, source, site, start_date, end_date, model, dir, username, con) {
  ...
  if (model == "SIPNET") {
    ...
    results <- data.frame(
      file = file.path(folder, "ED_MET_DRIVER_HEADER"),
      host = PEcAn.remote::fqdn(),
      mimetype = "text/plain",
    ...
    metinfo <- PEcAn.DB::db.query(paste0(
      "select mimetypes.type_string, formats.name from mimetypes, formats, modeltypes, modeltypes_formats",
      " where modeltype_id=modeltypes.id and format_id=formats.id and formats.mimetype_id=mimetypes.id",
      " and tag='met' and modeltypes.name='", model, "'"
    ), con)
