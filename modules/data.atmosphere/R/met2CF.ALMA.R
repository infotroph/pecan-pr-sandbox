...
met2CF.PalEONregional <- function(in.path, in.prefix, outfolder, start_date, end_date, overwrite = FALSE,
  ...
  for (year in start_year:end_year) {
    ...
    print(year)
    var <- ncdf4::ncvar_def(
      name = "air_temperature", units = "K", dim = dim,
    ...
    insertPmet(met[["swdown"]],
      nc2 = nc2, var2 = "surface_downwelling_shortwave_flux_in_air",
      units2 = "W m-2", dim2 = dim, verbose = verbose
    )

    # surface_downwelling_longwave_flux_in_air
    insertPmet(met[["lwdown"]],
      nc2 = nc2, var2 = "surface_downwelling_longwave_flux_in_air",
      units2 = "W m-2", dim2 = dim, verbose = verbose
    )
    ...
    insertPmet(met[["precipf"]],
      nc2 = nc2, var2 = "precipitation_flux", units2 = "kg/m^2/s",
...
met2CF.PalEON <- function(in.path, in.prefix, outfolder, start_date, end_date, lat, lon, overwrite = FALSE,
  ...
  for (year in start_year:end_year) {
    ...
    print(c(latlon, year))
    var <- ncdf4::ncvar_def(
      name = "latitude", units = "degree_north", dim = (list(lat, lon, time)),
      missval = as.numeric(-9999)
    )
    nc2 <- ncdf4::nc_create(filename = new.file, vars = var, verbose = verbose)
    ...
    var <- ncdf4::ncvar_def(
      name = "longitude", units = "degree_east", dim = (list(lat, lon, time)),
      missval = as.numeric(-9999)
    )
    nc2 <- ncdf4::ncvar_add(nc = nc2, v = var, verbose = verbose)
    ...
    # air_temperature
    insertPmet(met[["air_temperature"]],
      nc2 = nc2, var2 = "air_temperature", units2 = "K", dim2 = dim,
      verbose = verbose
    )

    # air_pressure
    insertPmet(met[["air_pressure"]],
      nc2 = nc2, var2 = "air_pressure", units2 = "Pa", dim2 = dim,
      verbose = verbose
    )
    ...
    # specific_humidity
    insertPmet(met[["specific_humidity"]],
      nc2 = nc2, var2 = "specific_humidity", units2 = "kg/kg", dim2 = dim,
      verbose = verbose
    )

    # surface_downwelling_shortwave_flux_in_air
    insertPmet(met[["surface_downwelling_shortwave_flux_in_air"]],
      nc2 = nc2, var2 = "surface_downwelling_shortwave_flux_in_air",
      units2 = "W m-2", dim2 = dim, verbose = verbose
    )

    # surface_downwelling_longwave_flux_in_air
    insertPmet(met[["surface_downwelling_longwave_flux_in_air"]],
      nc2 = nc2, var2 = "surface_downwelling_longwave_flux_in_air",
      units2 = "W m-2", dim2 = dim, verbose = verbose
    )
    ...
    # precipitation_flux
    insertPmet(met[["precipitation_flux"]],
      nc2 = nc2, var2 = "precipitation_flux", units2 = "kg/m^2/s",
      dim2 = dim, verbose = verbose
    )
...
met2CF.ALMA <- function(in.path, in.prefix, outfolder, start_date, end_date, overwrite = FALSE, verbose = FALSE) {
  ...
  for (year in start_year:end_year) {
    ...
    lon <- ncdf4::ncdim_def(name = "longitude", units = "", vals = 1:1, create_dimvar = FALSE)
    time <- ncdf4::ncdim_def(
      name = "time", units = tdim$units, vals = met[["time"]],
      create_dimvar = TRUE, unlim = TRUE
    )
    dim <- list(lat, lon, time)
    ...
    copyvals(
      ...
      dim2 = dim,
      conv = function(x) {
        PEcAn.utils::ud_convert(x, "degC", "K")
      },
      verbose = verbose
    )
    ...
    copyvals(
      ...
      dim2 = dim,
      conv = function(x) {
        PEcAn.utils::ud_convert(x, "kPa", "Pa")
      },
      verbose = verbose
    )
    ...
    copyvals(
      ...
      dim2 = dim, conv = function(x) {
        PEcAn.utils::ud_convert(x, "mol/mol", "ppm")
      },
      verbose = verbose
    )
    ...
    var <- ncdf4::ncvar_def(
      name = "specific_humidity", units = "kg/kg", dim = dim, missval = -6999,
      verbose = verbose
    )
    nc2 <- ncdf4::ncvar_add(nc = nc2, v = var, verbose = verbose)
    ...
    copyvals(
      ...
      dim2 = dim,
      conv = function(x) {
        PEcAn.utils::ud_convert(x, "umol m-2 s-1", "mol m-2 s-1")
      },
      verbose = verbose
    )
