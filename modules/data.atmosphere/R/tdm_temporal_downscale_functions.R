...
temporal.downscale.functions <- function(
  ...
  # downscaling
  dat.info <- c(
    "time.day", "year", "doy", "hour", "air_temperature_max.day",
  ...
  for (v in vars.list) {
    ...
    if (parallel) {
      ...
      if (v == "surface_downwelling_shortwave_flux_in_air") {
        mod.out <- parallel::mclapply(dat.list, model.train,
          mc.cores = n.cores,
      ...
      for (i in names(mod.out)) {
        ...
        outfile <- file.path(path.out, paste0(
          "betas_", v, "_",
          i, ".nc"
        ))
        dimY <- ncdf4::ncdim_def(paste0("coeffs_", i),
          units = "unitless",
          longname = "model.out coefficients", vals = 1:ncol(mod.out[[i]][["betas"]])
        )
        dimX <- ncdf4::ncdim_def("random",
          units = "unitless",
          longname = "random betas", vals = 1:nrow(mod.out[[i]][["betas"]])
        )
        var.list <- ncdf4::ncvar_def(i,
          units = "coefficients",
          dim = list(dimX, dimY), longname = paste0(
            "day ", i,
        ...
        save(mod.save, file = file.path(path.out, paste0(
          "model_",
          v, "_", i, ".Rdata"
        )))
      ...
      for (i in names(dat.list)) {
        if (v == "surface_downwelling_shortwave_flux_in_air") {
          mod.out <- model.train(
            dat.subset = dat.list[[i]], n.beta = n.beta, v = v,
        ...
        var.list <- ncdf4::ncvar_def(paste(i),
          units = "coefficients", dim = list(dimX, dimY),
