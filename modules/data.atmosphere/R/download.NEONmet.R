...
download.NEONmet <- function(sitename, outfolder, start_date, end_date,
  ...
  for (current_year in all_years) {
    ...
    if (nairTemp3 > nairTemp) {
      ...
      ncdata <- neonmet.getVals(
        dates = airTemp3Dates, product = availProducts[airTemp3Loc[1]], site = site,
      ...
      ncdata <- neonmet.getVals(
        dates = airTempDates, product = availProducts[airTempLoc[1]], site = site,
    ...
    if (length(pSurfDates) > 0) {
      ...
      ncdata <- neonmet.getVals(
        dates = pSurfDates, product = availProducts[pSurfLoc[1]], site = site,
    ...
    if (length(PARDates) > 0) {
      ...
      PAR.var <- ncdf4::ncvar_def(
        name = "surface_downwelling_photosynthetic_photon_flux_in_air",
        units = "mol m-2 s-1",
        dim = xytdim
      )
      nc <- ncdf4::ncvar_add(nc = nc, v = PAR.var, verbose = verbose)
      ncdata <- neonmet.getVals(
        dates = PARDates, product = availProducts[PARLoc[1]], site = site,
    ...
    if (length(precipDates) > 0) {
      ...
      ncdata <- neonmet.getVals(
        dates = precipDates, product = availProducts[precipLoc[1]], site = site,
        datetime = datetime, data_col = "priPrecipBulk", QF_col = "priPrecipFinalQF",
        urlstring = "\\.00000\\.900\\.(.*)30min",
        units = c("kg m-2 1/1800 s-1", "kg m-2 s-1")
      ) # mm per half hour
    ...
    if (length(RHDates) > 0) {
      ...
      ncdata <- neonmet.getVals(
        dates = RHDates, product = availProducts[RHLoc[1]], site = site,
    ...
    if (length(SWLWDates) > 0) {
      ...
      if (length(SWDates) > 0) {
        ...
        nc <- ncdf4::ncvar_add(nc = nc, v = SW.var, verbose = verbose)
        ncdata <- neonmet.getVals(
          dates = SWDates, product = availProducts[SWLoc[1]], site = site,
    ...
    if (length(WSpdDates) > 0) {
      ...
      WSpd.var <- ncdf4::ncvar_def(
        name = "wind_speed",
        units = "m s-1",
        dim = xytdim
      )
      WDir.var <- ncdf4::ncvar_def(
        name = "wind_direction",
        units = "degrees",
        dim = xytdim
      )
    ...
    if (length(soilTDates > 0)) {
      ...
      ncdata <- neonmet.getVals(
        dates = soilTDates, product = availProducts[soilTLoc[1]], site = site,
