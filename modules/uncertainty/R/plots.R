...
plot_variance_decomposition <- function(plot.inputs,
  ...
  base.plot <- ggplot2::ggplot(plot.data) +
    ggplot2::coord_flip()
...
plot_sensitivity <- function(sa.sample, sa.spline, trait, y.range = c(0, 50), median.i = 4,
  ...
  saplot <- saplot + ## plot spline function
    ...
    ggplot2::geom_point(ggplot2::aes(x = .data$x, y = .data$y),
      data = data.frame(x = sa.sample, y = sa.spline(sa.sample)),
      size = dotsize
    ) + # indicate median with larger point
    ggplot2::geom_point(ggplot2::aes(x = .data$x, y = .data$y),
      data = data.frame(x = sa.sample[median.i], y = sa.spline(sa.sample[median.i])),
      size = dotsize * 1.3
    ) +
    ...
    ggplot2::theme(
      axis.text.x = ggplot2::element_text(size = fontsize$axis),
  ...
  if (!is.null(prior.sa.sample) & !is.null(prior.sa.spline)) {
    ...
    saplot <- saplot + ## plot spline
      ggplot2::geom_line(ggplot2::aes(x = .data$x, y = .data$y),
        data = data.frame(x = prior.x, y = prior.sa.spline(prior.x)),
        size = linesize, color = "grey"
      ) + ## plot points used to evaluate spline
      ggplot2::geom_point(ggplot2::aes(x = .data$x, y = .data$y),
        data = data.frame(x = prior.sa.sample, y = prior.sa.spline(prior.sa.sample)),
        size = dotsize, color = "grey"
      ) + ## indicate location of medians
      ggplot2::geom_point(ggplot2::aes(x = .data$x, y = .data$y),
        data = data.frame(x = prior.sa.sample[median.i], y = prior.sa.spline(prior.sa.sample[median.i])),
...
plot_sensitivities <- function(sensitivity.plot.inputs,
  ...
  for (trait in traits) {
    ...
    sensitivity.plots[[trait]] <- plot_sensitivity(
      sa.sample = sa.samples[, trait],
