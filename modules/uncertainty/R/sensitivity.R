      ...
      for (quantile.str in quantiles.str) {
        if (quantile.str != MEDIAN) {
          ...
          if (!is.null(con)) {
            ...
            insert_result <- PEcAn.DB::db.query(paste0(
              "INSERT INTO runs (model_id, site_id, start_time, finish_time, outdir, ensemble_id, parameter_list) values ('",
              settings$model$id, "', '",
              settings$run$site$id, "', '",
              settings$run$start.date, "', '",
              settings$run$end.date, "', '",
            ...
            for (pft in defaults) {
              PEcAn.DB::db.query(paste0(
                "INSERT INTO posteriors_ensembles (posterior_id, ensemble_id) values (",
                pft$posteriorid, ", ",
                ensemble.id, ");"
              ), con = con)
            }
