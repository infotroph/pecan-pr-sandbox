...
read.ameriflux.L2 <- function(file.name, year) {
  data <- as.data.frame(utils::read.table(file.name,
    header = TRUE, sep = ",",
...
flux.uncertainty <- function(measurement, QC = 0, flags = TRUE, bin.num = 10,
  ...
  for (k in 1:(length(bins) - 1)) {
    ...
    if (nBin[k] > minBin) {
      ...
      print(paste(
        length(magnitude[use]), sum(!is.na(change[use])),
        magBin[k], errBin[k]
      ))
    } else {
      if (nBin[k] == 0) {
...
plot.oechel.flux <- function(observations, site) {
  ...
  print("NEE+")
  plot_flux_uncertainty((observations$FC[observations$FC >= 0]),
    flags = flags,
  ...
  print("LE")
  plot_flux_uncertainty((observations$LE[observations$LE >= 0]),
    flags = flags,
  ...
  plot_flux_uncertainty(observations$TS1[observations$TS1 >= 0],
    flags = flags,
...
tundra.flux.uncertainty <- function() {
  ...
  itex.climate <- lapply(1998:2011, function(year) {
    ...
    columns <- c(
      "UTC", "ATZ", "location", "site", "treatment", "plot",
