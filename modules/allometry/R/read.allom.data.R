...
read.allom.data <- function(pft.data, component, field, parm, nsim = 10000) {
  ...
  if (!is.null(parm)) {
    ...
    for (i in seq_len(nrow(allom$parm))) {
    ...
    rng <- rbind(
      nu(allom$parm$MinDiameter[sel]),
      nu(allom$parm$MaxDiameter[sel])
    ) ## units = cm
    ...
    allomParms <- as.data.frame(cbind(
      a, b, c, d, e, se, R2, Rratio, cf, eqn, n,
      Xmin, Xmax, Xcor, Ycor, Xtype, cite, spp
    ))
