...
InventoryGrowthFusion <- function(data, cov.data = NULL, time_data = NULL, n.iter = 5000, n.chunk = n.iter, n.burn = min(n.chunk, 2000), random = NULL, fixed = NULL, time_varying = NULL, burnin_plot = FALSE, save.jags = "IGF.txt", z0 = NULL, save.state = TRUE, restart = NULL) {
  ...
  check.dup.data <- function(data, loc) {
    if (any(duplicated(names(data)))) {
      PEcAn.logger::logger.error("duplicated variable at", loc, names(data))
    }
  }
...
model{
  ...
  if (is.null(fixed)) {
    ...
    X.terms <- sapply(X.terms, function(str) {
      unlist(strsplit(str, , split = "*", fixed = TRUE))
    })
    X.terms <- which(sapply(X.terms, function(x) {
      any(toupper(x) == "X")
    }))
    if (length(X.terms) > 0) {
  ...
  for (k in avail.chunks) {
    ...
    D <- coda::as.mcmc.list(lapply(jags.out, function(x) {
      x[, "deviance"]
    }))
    gbr <- coda::gelman.diag(D)$psrf[1, 1]
    trend <- mean(sapply(D, function(x) {
      stats::coef(stats::lm(x ~ seq_len(n.chunk)))[2]
    }))
