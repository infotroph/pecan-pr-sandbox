...
soilgrids_soilC_extract <- function(site_info, outdir = NULL, verbose = TRUE) {
  ...
  internal_site_info <- data.frame(
    site_info$site_id[-na.ind],
    site_info$site_name[-na.ind],
    site_info$lat[-na.ind],
    site_info$lon[-na.ind]
  ) %>% `colnames<-`(names(site_info))
  ...
  mean_site.2 <- data.frame(
    site_id = internal_site_info$site_id,
  ...
  soilgrids_soilC_data <- data.frame(
    internal_site_info$site_id,
