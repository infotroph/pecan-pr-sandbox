...
fia.to.psscss <- function(settings,
  ...
  for (pft in settings$pfts) {
    if (is.null(query)) {
      query <- paste0(
        "SELECT bp.name as pft, bs.spcd FROM pfts as bp INNER JOIN ",
  ...
  query <- paste(
    "SELECT p.cycle, p.statecd, p.measyear as time, p.cn as patch, ",
  ...
  ##################
  query <- paste0(
    "SELECT p.measyear as time,p.cycle,p.statecd,p.cn as patch,
  ...
  if (nrow(css) == 0) {
    PEcAn.logger::logger.severe(paste0(
      "No trees remain for selected PFTs. ",
