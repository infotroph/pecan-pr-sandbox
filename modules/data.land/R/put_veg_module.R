...
put_veg_module <- function(getveg.id, dbparms,
  ...
  if (!is.null(input_veg$output)) {
    model_info <- PEcAn.DB::db.query(paste0(
      "SELECT f.name, f.id, mt.type_string from modeltypes as m", " join modeltypes_formats as mf on m.id = mf.modeltype_id",
      " join formats as f on mf.format_id = f.id", " join mimetypes as mt on f.mimetype_id = mt.id",
      " where m.name = '", model, "' AND mf.tag='", input_veg$output, "'"
    ), con)
  ...
  putveg.id <- PEcAn.DB::convert_input(
    input.id = getveg.id$input.id,
