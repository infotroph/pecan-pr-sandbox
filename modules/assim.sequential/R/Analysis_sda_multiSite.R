...
EnKF.MultiSite <- function(settings, Forecast, Observed, H, extraArg = NULL, ...) {
  ...
  dots <- list(...)
  if (length(dots) > 0) {
    lapply(names(dots), function(name) {
  ...
  if (censored.data && is.null(extraArg$Pf)) {
    ...
    if (t == 1) {
      ...
      data.tobit2space <- list(
        ...
        Sigma_0 = solve(diag(1000, length(mu.f)))
      ...
      for (j in seq_along(mu.f)) {
        for (n in seq_len(nrow(X))) {
          ...
          conf_tobit2space$addSampler(node, "toggle", control = list(
            type =
              "RW"
          ))
        }
      ...
      for (i in seq_along(X)) {
        ...
        valueInCompiledNimbleFunction(
          Cmcmc_tobit2space$samplerFunctions[[samplerNumberOffset_tobit2space + i]],
          "toggle", 1 - x.ind[i]
        )
      }
    } else {
  ...
  if (t == 1) {
    ...
    if (ncol(aqq) > 1 & nrow(aqq) > 1) {
      aqq[, , t] <- Local.support(
        aqq[, , t],
        distances[
          ceiling(elements.W.Data / length(var.names)), # finding sites with data
