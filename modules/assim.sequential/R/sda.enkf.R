...
sda.enkf.original <- function(settings, obs.mean, obs.cov, IC = NULL, Q = NULL, adjustment = TRUE, restart = NULL) {
  ...
  var.names <- unlist(
    sapply(settings$state.data.assimilation$state.variable,
      function(x) {
        x$variable.name
      },
      USE.NAMES = FALSE
  ...
  for (i in seq_len(nens)) {
    if (no_split) { # currently this is only for ED2, ensemble generator + refactoring will change these soon anyway
      ...
      inputs[[i]] <- lapply(settings$run$inputs, function(x) {
        return(x %>% purrr::map(function(inputs) {
          return((inputs %>% unlist())[i])
        }))
      })
  ...
  for (i in seq_len(nens)) {
    ...
    ## Write Configs
    if (is.null(restart)) {
      do.call(what = my.write.config, args = list(
        defaults = NULL,
  ...
  interval <- NULL
  state.interval <- cbind(
    as.numeric(lapply(settings$state.data.assimilation$state.variables, "[[", "min_value")),
  ...
  for (t in seq_len(nt)) { #
    ...
    for (i in seq_len(nens)) {
      X_tmp[[i]] <- do.call(my.read_restart, args = list(
        outdir = outdir,
        runid = run.id[[i]],
    ...
    if (any(obs)) {
      ...
      if (FALSE) { # interactive() & t > 1
        t1 <- 1
        names.y <- unique(unlist(lapply(obs.mean[t1:t], function(x) {
          names(x)
        })))
        ...
        for (i in sample(x = 1:ncol(X), size = 2)) {
          ...
          Xci <- plyr::laply(FORECAST[t1:t], function(x) {
            stats::quantile(x[, i], c(0.025, 0.975))
          })
          ...
          if (i <= ncol(Ybar)) {
            ...
            graphics::lines(as.Date(obs.times[t1:t]), as.numeric(Ybar[, i]),
              type = "l",
              col = "darkgreen", lwd = 2
            )
          }
      ...
      if (processvar == FALSE) {
        ...
        for (i in 1:length(var.names)) {
          intervalX[which(startsWith(
            rownames(intervalX),
            var.names[i]
          )), ] <- matrix(
            c(
              as.numeric(settings$state.data.assimilation$state.variables[[i]]$min_value),
        ...
        if (recompile == TRUE) {
          ...
          # set.seed(0)
          # ptm <- proc.time()
          tobit2space_pred <- nimble::nimbleModel(tobit2space.model,
        ...
        if (recompile == TRUE) { # TO DO need to make something that works to pick weather to compile or not
          ...
          dimensions.tobit <- list(
            X = length(mu.f), X.mod = ncol(X),
          ...
          inits.pred <- list(
            q = diag(length(mu.f)), X.mod = as.vector(mu.f),
            X = stats::rnorm(length(mu.f), 0, 1)
          ) #

          model_pred <- nimble::nimbleModel(tobit.model,
            data = data.tobit, dimensions = dimensions.tobit,
    ...
    if (processvar == TRUE) {
      for (i in 1:ncol(analysis)) {
        int.save <- state.interval[which(startsWith(
          colnames(analysis)[i],
          var.names
        )), ]
    ...
    if (interactive() & t > 1) { #
      ...
      for (i in 1:ncol(FORECAST[[t]])) { #
        ...
        ylab.names <- unlist(sapply(
          settings$state.data.assimilation$state.variable,
          function(x) {
            x
          }
        )[2, ], use.names = FALSE)
  ...
  if (nens > 1) {
    ...
    YCI <- YCI[, Y.order]
    Xsum <- plyr::laply(FORECAST, function(x) {
      mean(rowSums(x[, 1:length(names.y)], na.rm = TRUE))
    })[t1:t]
    Xasum <- plyr::laply(ANALYSIS, function(x) {
      mean(rowSums(x[, 1:length(names.y)], na.rm = TRUE))
    })[t1:t]
    ...
    for (i in seq_along(obs.mean[[1]])) {
      ...
      dat.stats <- data.frame(
        rmse = PEcAn.benchmark::metric_RMSE(dat),
